...
func find_item_by_path(parentComp: Computer, pathParsed: Array, pathIn: String):
	...
	for child in children:
		if is_instance_valid(child) && child._class == "Directory" && child.get_directory_name() == pathParsed[selfParsed.size()]:
			return child.find_item_by_path(parentComp, pathParsed, pathIn)
		elif is_instance_valid(child) && child._class == "File" && child.get_file_name() + child.get_extension() == pathParsed[selfParsed.size()]:
			return child
	return null
